...
**対応した問題**:
1. 長時間の動画解析が5分ごとに暗黙リセットされる
2. 動画→カメラ切替後にモード判定が壊れる
3. CSV/レポートの時間軸がモードごとに不一致
4. レポート平均値が初期ゼロ値に引っ張られる

**実施した修正**:
- ✅ メモリクリーンアップをアイドル時のみ実行（解析中セッションを中断しない）
- ✅ カメラ起動時に動画状態を明示的に解放
- ✅ CSVの時間列を`elapsed_ms`に統一
- ✅ レポート時間を実測経過時間から算出
- ✅ レポート平均値を有効データ中心で算出
- ✅ 前額面/矢状面でレポートの主要指標を切り替え
...
企業提供レベルへ引き上げるための第1段階として、以下の設計ドキュメントを追加しました。

- `docs/ENTERPRISE_QUALITY_GATE.md` - 公開品質の到達基準
- `docs/TARGET_ARCHITECTURE.md` - モジュール分割の目標構成
- `docs/METRIC_AND_REPORT_SPEC.md` - 指標とレポートの基準仕様

このリポジトリは現時点ではプロトタイプ由来の単一HTML構成であり、今後は上記ドキュメントに沿って段階的に分割・検証・製品化を進めます。
...
- ✅ **動画ファイル読み込み**
  ...
  - videoFileUrlを読み替え時/エラー時に安全に解放
  - 動画終了時の自動finalizeSession()呼び出し
- ✅ **動画解析開始**
  - 動画ファイル読み込み後に「▶ 開始」で解析開始
  - 再生速度変更UIは削除済み（常に通常速度）
  - 時間軸は`elapsed_ms`として統一保存
